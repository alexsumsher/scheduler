...
class sys_gardD(object):
	...
	def runner(self):
		...
		if self.wait_amount <= 0:
			...
			loger.info('run event(%s): %s' % (_checkdt, e0.name))
			action = e0.act
			...
			self.nextwaiting = self.timeslot
		#loger.info(["%s:%s" % (e.name, e.until_next_secs) for e in self.events])
		#loger.info(f"wait: {self.wait_amount} to event: {self.e0.name}")
		# looper
		self._rerun()
	...
	def drop_event(self, ename):
		for i in range(len(self.events)):
			if self.events[i].name == ename:
				self.events.pop(i)
				loger.info(f"event {ename} is droped.")
				return True
	...
	def quick_insert_event(self, event, now=None):
		...
		for i in range(len(self.events)):
			...
			if e.until_next_secs > etime:
				...
				loger.info(f"q insert event {event.name} at positioin: {i}")
				return
		self.events.append(event)
		#print(self)
		loger.info(f"{event.name} append to events.")
	...
	def add_event(self, event):
		...
		loger.info(f"new event {event.name}")
		if len(self.events) == 0:
			self.events.append(event)
			return
